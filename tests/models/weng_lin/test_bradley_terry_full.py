"""
All tests for the BradleyTerryFull model are located here.
"""
...
from openskill.models.weng_lin import BradleyTerryFull
...
def test_model_defaults() -> None:
    ...
    assert model.__repr__() == f"BradleyTerryFull(mu=25.0, sigma={25.0 / 3.0})"
    assert model.__str__() == (
        f"Bradley-Terry Full Pairing Model Parameters: \n\n"
        f"mu: 25.0\n"
        f"sigma: {25.0 / 3.0}\n"
    )
...
def test_rating_defaults() -> None:
    ...
    # Default values
    model = BradleyTerryFull(mu=30, sigma=30 / 3)
    rating = model.rating()
    assert rating.mu == 30.0
    assert rating.sigma == 30.0 / 3.0
    assert rating.__repr__() == f"BradleyTerryFullRating(mu=30.0, sigma={30.0/3.0})"
    assert rating.__str__() == (
        f"Bradley-Terry Full Pairing Player Data: \n\n"
        f"id: {rating.id}\n"
        f"mu: 30.0\n"
        f"sigma: {30.0/3.0}\n"
    )
...
def test_rating_overrides() -> None:
    ...
    assert rating_2.__repr__() == f"BradleyTerryFullRating(mu=30.0, sigma={40.0/3.0})"
...
def test_bradley_terry_full_team_rating() -> None:
    """
    Tests the :code:`BradleyTerryFullTeamRating` class.
    """

    # Test Constructor
    team = [BradleyTerryFull().rating() for _ in range(5)]
    rating = BradleyTerryFullTeamRating(1, 2, team, 3)
    assert rating.mu == 1
    ...
    # Test Equality
    assert rating == BradleyTerryFullTeamRating(1, 2, team, 3)
    assert rating != BradleyTerryFullTeamRating(2, 2, team, 3)
    assert rating != BradleyTerryFullTeamRating(1, 3, team, 3)
    assert rating != BradleyTerryFullTeamRating(1, 2, [BradleyTerryFull().rating()], 3)
    assert rating != BradleyTerryFullTeamRating(1, 2, team, 4)
    assert rating != 2

    # Test Hash
    assert hash(rating) == hash(BradleyTerryFullTeamRating(1, 2, team, 3))
    assert hash(rating) != hash(BradleyTerryFullTeamRating(2, 2, team, 3))
    assert hash(rating) != hash(BradleyTerryFullTeamRating(1, 3, team, 3))
    assert hash(rating) != hash(
        BradleyTerryFullTeamRating(
            1, 2, [BradleyTerryFullRating(mu=25.0, sigma=25.0 / 3)], 3
        )
    )
    assert hash(rating) != hash(BradleyTerryFullTeamRating(1, 2, team, 4))
...
def test_c() -> None:
    """
    Ensure the c function from Bradley-Terry Full Pairing works as expected.
    """
    model = BradleyTerryFull()
    r = model.rating
    team_1 = [r()]
...
def test_a():
    """
    Ensure the `a` function from Bradley-Terry Full Pairing works as expected.
    """
    model = BradleyTerryFull()
    r = model.rating
    team_1 = [r()]
...
def test_rate() -> None:
    ...
    model = BradleyTerryFull(mu, sigma)
    r = model.rating
    team_1 = [r()]
    team_2 = [r(), r()]
    ...
    model = BradleyTerryFull(mu, sigma, balance=True)
...
def test_weight_bounds_custom() -> None:
    ...
    """
    model_default = BradleyTerryFull(weight_bounds=(1.0, 2.0))
    ...
    model_narrow = BradleyTerryFull(weight_bounds=(0.9, 1.1))
...
def test_weight_bounds_none_disables_normalization() -> None:
    ...
    """
    model = BradleyTerryFull(weight_bounds=None)
