...
def test_bradley_terry_part_team_rating() -> None:
    ...
    assert str(rating) == (
        "BradleyTerryPartTeamRating Details:\n\n"
        "mu: 1.0\n"
        "sigma_squared: 2.0\n"
        "rank: 3\n"
    )
...
def test_weight_bounds_custom() -> None:
    ...
    """
    model_default = BradleyTerryPart(weight_bounds=(1.0, 2.0))
    ...
    model_narrow = BradleyTerryPart(weight_bounds=(0.9, 1.1))
...
def test_weight_bounds_none_disables_normalization() -> None:
    ...
    """
    model = BradleyTerryPart(weight_bounds=None)
