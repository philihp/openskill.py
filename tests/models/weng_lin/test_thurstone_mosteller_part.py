...
def test_model_defaults() -> None:
    ...
    assert model.__repr__() == f"ThurstoneMostellerPart(mu=25.0, sigma={25.0 / 3.0})"
...
def test_rating_defaults() -> None:
    ...
    # Default values
    model = ThurstoneMostellerPart(mu=30, sigma=30 / 3)
    rating = model.rating()
    assert rating.mu == 30.0
    assert rating.sigma == 30.0 / 3.0
    assert (
        rating.__repr__() == f"ThurstoneMostellerPartRating(mu=30.0, sigma={30.0/3.0})"
    )
...
def test_rating_overrides() -> None:
    ...
    assert (
        rating_2.__repr__()
        == f"ThurstoneMostellerPartRating(mu=30.0, sigma={40.0/3.0})"
    )
...
def test_thurstone_mosteller_part_team_rating() -> None:
    """
    Tests the :code:`ThurstoneMostellerPartTeamRating` class.
    """

    # Test Constructor
    team = [ThurstoneMostellerPart().rating() for _ in range(5)]
    rating = ThurstoneMostellerPartTeamRating(1, 2, team, 3)
    assert rating.mu == 1
    ...
    # Test Equality
    assert rating == ThurstoneMostellerPartTeamRating(1, 2, team, 3)
    assert rating != ThurstoneMostellerPartTeamRating(2, 2, team, 3)
    assert rating != ThurstoneMostellerPartTeamRating(1, 3, team, 3)
    assert rating != ThurstoneMostellerPartTeamRating(
        1, 2, [ThurstoneMostellerPart().rating()], 3
    )
    assert rating != ThurstoneMostellerPartTeamRating(1, 2, team, 4)
    assert rating != 2

    # Test Hash
    assert hash(rating) == hash(ThurstoneMostellerPartTeamRating(1, 2, team, 3))
    assert hash(rating) != hash(ThurstoneMostellerPartTeamRating(2, 2, team, 3))
    assert hash(rating) != hash(ThurstoneMostellerPartTeamRating(1, 3, team, 3))
    assert hash(rating) != hash(
        ThurstoneMostellerPartTeamRating(
            1, 2, [ThurstoneMostellerPartRating(mu=25.0, sigma=25.0 / 3)], 3
        )
    )
    assert hash(rating) != hash(ThurstoneMostellerPartTeamRating(1, 2, team, 4))

    # Test String
    assert repr(rating) == "ThurstoneMostellerPartTeamRating(mu=1.0, sigma_squared=2.0)"
    assert str(rating) == (
        "ThurstoneMostellerPartTeamRating Details:\n\n"
        "mu: 1.0\n"
        "sigma_squared: 2.0\n"
        "rank: 3\n"
    )
...
def test_c() -> None:
    """
    Ensure the c function from Thurstone-Mosteller Partial Pairing works as expected.
    """
    model = ThurstoneMostellerPart()
    r = model.rating
    team_1 = [r()]
...
def test_a():
    """
    Ensure the `a` function from Thurstone-Mosteller Partial Pairing works as
    expected.
    """
    model = ThurstoneMostellerPart()
    r = model.rating
...
def check_expected(
    data, data_key: str, results: list[list[ThurstoneMostellerPartRating]]
) -> None:
    """
...
def test_rate() -> None:
    ...
    model = ThurstoneMostellerPart(mu, sigma)
    r = model.rating
    team_1 = [r()]
    team_2 = [r(), r()]
...
def test_weight_bounds_custom() -> None:
    ...
    """
    model_default = ThurstoneMostellerPart(weight_bounds=(1.0, 2.0))
    ...
    model_narrow = ThurstoneMostellerPart(weight_bounds=(0.9, 1.1))
...
def test_weight_bounds_none_disables_normalization() -> None:
    ...
    """
    model = ThurstoneMostellerPart(weight_bounds=None)
